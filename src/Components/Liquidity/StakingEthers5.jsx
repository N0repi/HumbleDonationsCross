...
export async function addLPtoPool(
    ...
    const { token0: amount0Desired, token1: amount1Desired } = await calculateLiquidityPosition(
        tokenQuantity
    )
    ...
    const gasLimitApprove0 = estimatedGasApprove0.add(estimatedGasApprove0.mul(30).div(100))
    const token0approve = await tokenContract0.approve(positionManagerAddress, amount1, {
    ...
    const gasLimitApprove1 = estimatedGasApprove1.add(estimatedGasApprove1.mul(30).div(100))
    const token1approve = await tokenContract1.approve(positionManagerAddress, amount0, {
    ...
    async function handleIncreaseLiquidity(
        ...
        try {
            if (!tokenId || !liquidity || !amount0 || !amount1) {
                console.error("Invalid arguments:", { tokenId, liquidity, amount0, amount1 })
...
function handleTransactionError(error) {
    ...
    if (error.code === ethers.errors.UNPREDICTABLE_GAS_LIMIT) {
        console.error("The transaction is likely to fail due to gas estimation issues.")
    } else {
...
async function incentiveToTokenId(tokenId, setConfirmationHash, setTransactionError) {
    const provider = await getConnectedSigner()
    const signer = await provider.getSigner()
    ...
    const iKeyInstance = new ethers.Contract(IKEY_ADDRESS, EncodeIncentiveUniV3ABIAbi, signer)
    ...
    const STFencoded = nonfungiblePositionManagerContract.interface.encodeFunctionData(
    ...
    const stakingMulticall = await nonfungiblePositionManagerContract["multicall(bytes[])"](
        stakingMulticallData
    )
    try {
...
export async function getRewards(tokenId) {
    ...
    const rewardInfo = await UniswapV3StakerInstance.getRewardInfo(decodedKey, tokenId)
...
export async function unstakeClaimRestake(
    ...
    const unstakeEncoded = UniswapV3StakerInstance.interface.encodeFunctionData("unstakeToken", [
        decodedKey,
        tokenId,
    ])

    const claimEncoded = UniswapV3StakerInstance.interface.encodeFunctionData("claimReward", [
        rewardToken,
        signerAddress,
        0,
    ])

    const stakeEncoded = UniswapV3StakerInstance.interface.encodeFunctionData("stakeToken", [
        decodedKey,
        tokenId,
    ])
    ...
    const multicallStakeClaimRestake = [unstakeEncoded, claimEncoded, stakeEncoded]
...
export async function unstakeClaimWithdraw(
    ...
    const withdrawEncoded = UniswapV3StakerInstance.interface.encodeFunctionData("withdrawToken", [
        tokenId,
        signerAddress,
        ethers.utils.hexlify([]),
    ])

    const multicallStakeClaimRestake = [unstakeEncoded, claimEncoded, withdrawEncoded]
    ...
    await removeLiquidity(tokenId, setApprovalHash, setConfirmationHash, setTransactionError)
}
...
export async function calculateReserves() {
    ...
    const reserve0 = poolData.liquidity.mul(sqrtPriceX96).div(ethers.BigNumber.from(2).pow(96))
    const reserve1 = poolData.liquidity.mul(ethers.BigNumber.from(2).pow(96)).div(sqrtPriceX96)
...
export async function removeLiquidity(
    ...
    let positionData = await nonfungiblePositionManagerContract.positions(tokenId)
    ...
    const decreaseTxEncoded = nonfungiblePositionManagerContract.interface.encodeFunctionData(
        "decreaseLiquidity",
        [decreaseParams]
    )
    ...
    const collectTxEncoded = nonfungiblePositionManagerContract.interface.encodeFunctionData(
        "collect",
        [collectParams]
    )
    ...
    try {
        try {
            ...
            const multicallCall = await nonfungiblePositionManagerContract["multicall(bytes[])"](
                multicallDecreaseCollect,
                { gasLimit: gasLimitMulticall }
            )
            await multicallCall.wait()
...
export async function IncreaseLiquidity(
    ...
    try {
        const increaseTx = await nonfungiblePositionManagerContract.increaseLiquidity(
            increaseParams,
            {
                gasLimit: 1000000,
            }
...
export async function DecreaseLiquidity(
    ...
    const approveEncoded = nonfungiblePositionManagerContract.interface.encodeFunctionData(
        "approve",
        [positionManagerAddress, tokenId]
    )

    const decreaseEncoded = nonfungiblePositionManagerContract.interface.encodeFunctionData(
        "decreaseLiquidity",
        [increaseParams]
    )
    ...
    const approveDecreaseCollectMulticallData = [approveEncoded, decreaseEncoded, collectTxEncoded]
    ...
    const estimatedGasMulticall2 = await nonfungiblePositionManagerContract.estimateGas[
        "multicall(bytes[])"
    ](approveDecreaseCollectMulticallData)
    const gasLimitMulticall2 = estimatedGasMulticall2.add(estimatedGasMulticall2.mul(20).div(100)) // Adding 20% buffer

    try {
        const approveDecreaseCollectMulticall = await nonfungiblePositionManagerContract[
            "multicall(bytes[])"
        ](approveDecreaseCollectMulticallData, { gasLimit: gasLimitMulticall2 })

        await approveDecreaseCollectMulticall.wait()
        setApprovalHash(approveDecreaseCollectMulticall.hash)
        console.log("approveDecreaseCollectMulticall:", approveDecreaseCollectMulticall)
    } catch (error) {
        if (error.transactionHash) {
            setTransactionError(error.transactionHash.hash)
        } else if (error.receipt && error.receipt.hash) {
    ...
    try {
        ...
        const estimatedGasMulticall3 = await nonfungiblePositionManagerContract.estimateGas[
            "multicall(bytes[])"
        ...
        const approveSTFmulticall = await nonfungiblePositionManagerContract["multicall(bytes[])"](
            approveSTFmulticallData,
            { gasLimit: gasLimitMulticall3 }
        )
...
export async function ReStake(tokenId, setApprovalHash, setConfirmationHash) {
    ...
    const reStakeTx = await UniswapV3StakerInstance.stakeToken(decodedKey, tokenId)
...
export async function calculateIncentiveAPR() {
    ...
    const EncodeIncentiveUniV3ABIAbi = EncodeIncentiveUniV3ABI
    const iKeyInstance = new ethers.Contract(IKEY_ADDRESS, EncodeIncentiveUniV3ABIAbi, provider)

    const rewardTokenAbi = ["function decimals() view returns (uint8)"]
    const rewardTokenContract = new ethers.Contract(rewardToken, rewardTokenAbi, provider)
    const rewardTokenDecimals = await rewardTokenContract.decimals()

    const decodedKey = await iKeyInstance.decode(iKey)
    const encodedKey = await iKeyInstance.compute(decodedKey)
    const stakesPerIncentives = await UniswapV3StakerInstance.incentives(encodedKey)
    const totalRewardUnclaimed = BigInt(stakesPerIncentives[0].toString())
    const totalRewardUnclaimedFormatted = Number(totalRewardUnclaimed) / 10 ** rewardTokenDecimals

    const incentiveDurationInSeconds = decodedKey[3].toNumber() - decodedKey[2].toNumber()
    const incentiveDurationInDays = incentiveDurationInSeconds / (60 * 60 * 24)

    const rewardPerDay = totalRewardUnclaimedFormatted / Number(incentiveDurationInDays)
    ...
    const wethPerLiquidityUSD = await getINtoUSD({ name: "Wrapped Ether" }, wethPerLiquidity)

    // Remove dollar sign from the return of getINtoUSD
    const hdtPerLiquidityUSDNumber = parseFloat(hdtPerLiquidityUSD.replace("$", ""))
    const wethPerLiquidityUSDNumber = parseFloat(wethPerLiquidityUSD.replace("$", ""))
    ...
    const rewardPerDayUSD = await getINtoUSD({ name: "Wrapped Ether" }, rewardPerDay)

    // Calculate for APR
    const calculateAPR = (rewardPerDayUSD, totalLiquidityUSD) => {
        const rewardPerDayUSDUSDNumber = parseFloat(rewardPerDayUSD.replace("$", ""))
        // console.log("----calulateAPR----")
        // console.log("rewardPerDayUSD:", rewardPerDayUSD)
        // console.log("totalLiquidityUSD", totalLiquidityUSD)
        const daysInYear = 365
        const apr = ((rewardPerDayUSDUSDNumber * daysInYear) / totalLiquidityUSD) * 100
        // console.log("apr:", apr)
